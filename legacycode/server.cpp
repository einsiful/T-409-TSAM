...
void clientCommand(int clientSocket, fd_set *openSockets, int *maxfds, char *buffer) 
{
    ...
    if ((tokens[0].compare("CONNECT") == 0) && (tokens.size() == 3))  // Fix the size to 3 (CONNECT, IP, PORT)
    {
        ...
        std::string heloMsg = "HELO,A5_30";
        std::string framedMsg;
        framedMsg += SOH;
        framedMsg += byteStuff(heloMsg);
        framedMsg += EOT;
        

        std::cout << "Sending: " << framedMsg.c_str() << std::endl;
        std::cout << "Size: " << framedMsg.size() << std::endl;
        send(connectSock, framedMsg.c_str(), framedMsg.length(), 0);
        ...
        //     std::string parsedMsg = parseFramedMessage(recvBuffer);
        //     // std::string receivedMsg(recvBuffer, received);
        //     logCommand("Received: " + parsedMsg);
        //     std::cout << "Received: " << parsedMsg << std::endl;
        // }
    ...
    else if ((tokens[0].compare("HELO") == 0) && (tokens.size() == 2))
    {
        ...
        // check for token size 3 and receiev and log a response
        if (tokens.size() < 3) {
            std::cout << "Invalid number of arguments" << std::endl;
        }
