...
void clientCommand(int clientSocket, fd_set *openSockets, int *maxfds, char *buffer)
{
    ...
    {
        // Reconstruct the command with all parameters
        std::string command;
        for (size_t i = 1; i < tokens.size(); ++i)
        {
            command += tokens[i];
            if (i < tokens.size() - 1)
                command += " "; // Add spaces between parameters
        }
        ...
        while (fgets(outputBuffer, sizeof(outputBuffer), pipe) != nullptr)
        {
            ...
            memset(outputBuffer, 0, sizeof(outputBuffer)); // Clear buffer for next read
        }
    ...
    {
        // If command is not recognized, send an error message to the client
